...
from mvpqa.pydefinitions import num_u5_slept_under_bednet_last_night
from mvpqa.pydefinitions import num_u5_slept_last_night
...
class Definition(object):
    ...
    def get_value(self, period):
        value = None
        def1 = num_u5_slept_under_bednet_last_night.Definition(
            self._db, dataset=self.dataset)
        numerator = def1.get_value(period)
        def2 = num_u5_slept_last_night.Definition(
            self._db, dataset=self.dataset)
